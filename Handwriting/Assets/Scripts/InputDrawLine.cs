...
public class InputDrawLine : MonoBehaviour
{
   ...
   // List<GameObject> subSampledLineList = new List<GameObject>();
    ...
    void Start()
    {
        ...
        char[] inputword = letter[letterNum/intputCount].ToCharArray();
        ...
        if(letterNum >= intputCount * letter.Length) {
            nextLetter.GetComponent<Text>().text = "종료";
            resultText.GetComponent<Text>().text = "수고하셨습니다!!\n";
        } else {
            nextLetter.GetComponent<Text>().text = letter[letterNum/intputCount];
        }
    }
    ...
    void SetResultText(string linePoints, string name) {
        ...
        if(linePoints == "") {
            ...
            // 저장 완료
            resultText.GetComponent<Text>().text = (letterNum%intputCount+1) + "번째\n 저장되었습니다.";
        }
    }
    ...
    void Sampling() {
        ...
        for(int i = 0; i < elementArray.Count; i++)
        {
            ...
            int t = 0;
            Debug.Log("획순 순서 : "+ elementArray[i] +"/" + i); 
            // 필기 데이터를 일정 크기로 맞추기
            foreach(GameObject line in lineList) {
    ...
    void DrawSample() {
        ...
        // Sample Area 중앙점
        float xSampleAreaCenter = Screen.width/2 + transform.Find("SampledArea").GetComponent<RectTransform>().anchoredPosition.x  * ratioWidth;
        float ySampleAreaCenter = Screen.height/2 + transform.Find("SampledArea").GetComponent<RectTransform>().anchoredPosition.y * ratioHeight;
        ...
        for(int k = 0; k < elementArray.Count; k++) {
            ...
            string reObjectName = "SampledArea" + (k + 3);
    ...
    void DrawCanvas (List<List<Vector2>> sampledSubList,string objectName) {
        ...
        foreach(List<Vector2> line in sampledSubList) {
            ...
            for (int i = 0; i < line.Count; i++) {
                ...
                else if(LineCount == 1){c1 = new Color(0, 1, 0, 1);}
                else if(LineCount == 2){c1 = new Color(0, 0, 1, 1);}
                else if(LineCount == 3){c1 = new Color(1, 1, 0, 1);}
                else if(LineCount == 4){c1 = new Color(0, 1, 1, 1);}
                else if(LineCount == 5){c1 = new Color(1, 0, 1, 1);}
                else if(LineCount == 6){c1 = new Color(0.5f, 0.5f, 0.5f, 1);}
      ...
      public static List<string> DivideHangul(char source)
    {
        ...
        else if("ㅘ".Equals(mNucleus)) {fullString.Add("ㅗ");fullString.Add("ㅏ");}
